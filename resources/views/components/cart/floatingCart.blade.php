...
<div class="fixed bottom-10 right-10 z-[60]" x-cloak x-show="true" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-10" x-transition:enter-end="opacity-100 translate-y-0">
    <div class="relative group">
        ...
        <button @click="showCart = true"
            ...
            <!-- Icon -->
            <svg class="w-7 h-7 transform transition-transform duration-300 group-hover:-rotate-12" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <!-- Tooltip Label -->
        <span class="absolute right-full top-1/2 -translate-y-1/2 mr-4 px-3 py-1 bg-white text-gray-700 text-sm font-bold rounded-xl shadow-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300 whitespace-nowrap pointer-events-none">
            Keranjang
        </span>
    </div>
</div>